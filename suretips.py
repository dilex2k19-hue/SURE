...
def save_html(matches, filename="index.html"):
    ...
    # newest first
    all_matches = sorted(matches, key=lambda m: parse_date_safe(m.get("date", "")), reverse=True)
    ...
    history_matches = [m for m in all_matches if m.get("date") not in {latest_date, second_latest_date}]
    ...
    <body>
        ...
        <script>
            ...
            function getVisibleCardsInActiveTab() {{
                const panel = document.getElementById(activeTab);
                return panel ? panel.querySelectorAll(".match-card") : [];
            }}
...
def scrape_vip_matches():
    ...
    all_matches.sort(key=parse_date_safe, reverse=True)
